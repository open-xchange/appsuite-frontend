    ...
    Tour.registry.add({
        ...
        new Tour({
            ...
            .step({ back: false, noAutoAlign: true })
                .title(gt('Navigation'))
                .content(gt('All apps can be found in the app menu on the top right.'))
            ...
            .step({ back: false, noAutoAlign: true })
                ...
                .hotspot('#io-ox-topbar-dropdown-icon .contact-picture', { top: 16, left: 16 })
                .spotlight('#topbar-settings-dropdown')
                .waitFor('#topbar-settings-dropdown')
                .on('wait', function () { $('#io-ox-topbar-dropdown-icon .dropdown-toggle').click(); $('#io-ox-topbar-dropdown-icon').attr('forceOpen', true); })
                .on('hide', function () { $('#io-ox-topbar-dropdown-icon .dropdown-toggle').click(); $('#io-ox-topbar-dropdown-icon').attr('forceOpen', false); })
                .end()
            .step({ back: false, noAutoAlign: true })
                .title(gt('Settings'))
                .content(gt('Customize the product to your needs in the settings area.'))
                .hotspot('#io-ox-settings-topbar-icon i', { top: 9, left: 8 })
                .spotlight('#io-ox-settings-topbar-icon')
                .end()
            .step({ back: false, noAutoAlign: true })
                .title(gt('Help'))
                .content(gt('If you need assistance or have further questions, you can use the help app.'))
                .hotspot('#io-ox-context-help-icon i', { top: 12, left: 6 })
                .spotlight('#io-ox-context-help-icon')
