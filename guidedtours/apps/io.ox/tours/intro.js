...
define('io.ox/tours/intro', [
    ...
    Tour.registry.add({
        ...
        .step()
            ...
            .on('before:show', function () { if ($('.launcher-dropdown:visible').length === 0) { $('#io-ox-launcher .btn').first().click(); $('#io-ox-launcher').attr('forceOpen', true); } })
            .content(gt('To launch an app click on an entry inside the app launcher menu.'))
            .hotspot('#io-ox-launcher>button>.fa')
            .on('hide', function () { if ($('.launcher-dropdown:visible').length === 1) { $('#io-ox-launcher .btn').first().click(); $('#io-ox-launcher').attr('forceOpen', false); } })
            .end()
        .step()
