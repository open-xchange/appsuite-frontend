...
Scenario('[OXUIB-768] Follow up with Zoom integration', async ({ I, calendar, users }) => {
    ...
    I.say('Prevent adaption of location and untouched dial-in information');
    I.say('Connecting Zoom integration');
    ...
    I.fillField('Subject', 'OXUIB-768');
    I.fillField('Description', 'OXUIB-768');
    I.click('Copy link to location', calendar.locators.edit);
    I.click('Copy dial-in information to description', calendar.locators.edit);
    ...
    I.waitForDetached(calendar.locators.edit, 5);
    ...
    const conference = await I.grabValueFrom('.io-ox-calendar-edit-window select[name="conference-type"]');
    expect(conference).to.equal('none');

    let location = await I.grabValueFrom('.io-ox-calendar-edit-window input[name="location"]');
    expect(location).to.be.empty;

    let description = await I.grabValueFrom('.io-ox-calendar-edit-window textarea[name="description"]');
    expect(description).to.equal('OXUIB-768');

    I.dontSee('The appointment description might contain outdated information', calendar.locators.edit);

    I.click('Discard', calendar.locators.edit);
    I.waitForDetached(calendar.locators.edit, 5);

    I.say('Keep non-zoom location and modified dial-in information');
    I.click('Edit', '.io-ox-sidepopup');
    I.waitForText('Location', 5, calendar.locators.edit);
    ...
    I.waitForDetached(calendar.locators.edit, 5);
    I.wait(1);
    I.waitForText('Follow-up', 5, '.io-ox-sidepopup');
    I.click('Follow-up');
    I.waitForElement('.io-ox-calendar-edit-window select[name="conference-type"]', 5);

    location = await I.grabValueFrom('.io-ox-calendar-edit-window input[name="location"]');
    expect(location).to.equal('OXUIB-768');
    description = await I.grabValueFrom('.io-ox-calendar-edit-window textarea[name="description"]');
    expect(description).to.equal(newDescription);

    I.see('The appointment description might contain outdated information', calendar.locators.edit);
});
