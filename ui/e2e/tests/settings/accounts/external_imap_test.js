...
Scenario('[C7836] Add custom mail account (IMAP)', async ({ I, users, mail, dialogs, contexts }) => {
    ...
    I.fillField('Your password', externalUser.get('password'));
    dialogs.clickButton('Add');
    dialogs.clickButton('Configure manually');

    dialogs.waitForVisible();
    I.fillField('Account name', externalUser.get('primaryEmail'));
    I.fillField('Email address', externalUser.get('primaryEmail'));
    I.fillField('#transport_server', 'localhost');
    I.fillField('#mail_server', 'localhost');
    ...
    mail.newMail();
    I.fillField('To', externalUser.get('primaryEmail'));
    I.fillField('Subject', 'Howdee yihaa!');
    I.fillField({ css: 'textarea.plain-text' }, 'We are not in Cansas anymore!');
    mail.send();
    I.waitForText(externalUser.get('primaryEmail'));
    I.doubleClick(externalUser.get('primaryEmail'));
