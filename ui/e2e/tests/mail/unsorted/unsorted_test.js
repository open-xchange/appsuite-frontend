...
Scenario('[C8831] Add recipient manually', async ({ I, users, mail }) => {
    let [user] = users;
    await I.haveSetting('io.ox/mail//messageFormat', 'text');
    I.login('app=io.ox/mail', { user });
    mail.newMail();
...
Scenario('[C12118] Remove recipients', async ({ I, users, mail }) => {
    let [user] = users;
    await I.haveSetting('io.ox/mail//messageFormat', 'text');
    I.login('app=io.ox/mail', { user });
    mail.newMail();
...
Scenario('[C12119] Edit recipients', async ({ I, users, mail }) => {
    let [user] = users;
    await I.haveSetting('io.ox/mail//messageFormat', 'text');
    I.login('app=io.ox/mail', { user });
    mail.newMail();
...
Scenario('[C83384] Automatically bcc all messages', async ({ I, mail, users }) => {
    await Promise.all([
        I.haveSetting('io.ox/mail//messageFormat', 'text'),
        users.create()
    ]);
