...
Scenario('Compose new mail with signature below correctly placed initially', async ({ I }) => {
    for (let signature of signatures) {
        var response = await I.haveSnippet(signature);
        signature.id = response.data;
    }
...
Scenario('Reply to mail with signature above correctly placed and changed', async ({ I, users, mail }) => {
    ...
    await selectAndAssertSignature(I, mail, 'No signature', new RegExp('^' + emptyLine + '<blockquote type="cite">.*</blockquote>$'));
    ...
    await selectAndAssertSignature(I, mail, 'No signature', new RegExp('^' + someUserInput + '<blockquote type="cite">.*</blockquote>$'));
...
Scenario('Reply to mail with signature below correctly placed initially', async ({ I, users }) => {
    let [user] = users;
...
Scenario('[OXUIB-370] Default font style is used after appending signature', async ({ I, mail }) => {
    ...
    await selectAndAssertSignature(I, mail, 'First signature below',
        new RegExp('^<div class="default-style"(?: (?:data-mce-)?style="font-size: 10pt; font-family: arial black,avant garde; color: #0000ff;"){2}><br></div>' +
