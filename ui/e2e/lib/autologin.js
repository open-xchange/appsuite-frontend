...
const debug = require('codeceptjs').output.debug;
...
module.exports = function (config) {
    ...
    const evalUsers = typeof config.users === 'function';
    if (typeof config.users === 'object') {
        Object.keys(config.users).forEach(u => Object.assign(config.users[u], defaultUser, config.users[u]));
    }
    ...
    const loginFunction = async (name) => {
        const userSession = evalUsers ? config.users(name) : config.users[name];
