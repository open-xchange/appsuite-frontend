 ...
 */
 ...
 /**
 * MODIFICATIONS
 * As required per APL2.0, we would like to inform you that this
 * file has been modified:

 * 1) In addition to the handling of onShow, we introduced
 * onShowDeferred, which hands over the execution of showing the bubble
 * to another method that is contained in onShowDeferred. This way, you
 * can work with deferreds, for example wait for another module to load,
 * before showing the bubble.
 * 2) target can also be a function. The effect of this is it being
 * evaluated later, for example once something is loaded that modifies
 * the DOM tree.
 */
  ...
  utils = {
    ...
    getStepTarget: function(step) {
      ...
      if (typeof step.target === 'string') {
        //Just one target to test. Check, and return its results. no caching anymore
        return utils.getStepTargetHelper(step.target);
      }
      else if (typeof step.target === 'function') {
        var tempTarget = step.target();
  ...
  Hopscotch = function(initOptions) {
    ...
    showStepHelper = function(stepNum) {
      ...
      showBubble = function() {
        ...
        if(!!step.onShowDeferred) {
          step.onShowDeferred(function () { bubble.show(); });
        } else {
          bubble.show();
        }
      };
