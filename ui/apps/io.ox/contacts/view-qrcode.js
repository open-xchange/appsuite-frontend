...
define("io.ox/contacts/view-qrcode", [], function () {
    ...
    return {
        ...
        getVCard: function (data) {
            ...
            var c = function (s) {
                return s;
                return s.replace(/[äöüßÖÄÜ]/g, function (m) {
                    return umlaut[m];
                });
            };
            var add = function (s) {
                s = c(s);
                return (str.length + s.length) < 2000 ? s : "";
            };
            var phone = function (type, value) {
                if (value) {
                    return "TEL;TYPE=voice," + type.toLowerCase() + ":" + value + CRLF;
                } else {
                    return "";
                }
            };
            ...
            // header
            str = "BEGIN:VCARD" + CRLF + "VERSION:3.0" + CRLF;
            // full name
            str += t("N:" + c(data.last_name || "") + ";" + c(data.first_name || "") + ";;" + c(data.title || ""));
            str += "FN:" + c(ajoin(" ", data.title, data.first_name, data.last_name)) + CRLF;
            // company
            if (data.company || data.department) {
                str += "ORG:" + t((data.company || "") + ";" + (data.department || ""));
            }
            // email
            if (data.email1) {
                str += "EMAIL;TYPE=INTERNET:" + data.email1 + CRLF;
                mails[data.email1] = true;
            }
            if (data.email2 && mails[data.email2] !== true) {
                str += "EMAIL;TYPE=INTERNET:" + data.email2 + CRLF;
                mails[data.email2] = true;
            }
            if (data.email3 && mails[data.email3] !== true) {
            ...
            str += add(phone("work", data.telephone_business2));
            str += add(phone("home", data.telephone_home1));
            str += add(phone("home", data.telephone_home2));
            // title
            if (data.position) {
                str += add("TITLE:" + data.position + CRLF);
            }
            // role
            if (data.profession) {
                str += add("ROLE:" + data.profession + CRLF);
            }
            // birthday
            var d = new Date(data.birthday);
            if (!isNaN(d.getDate())) {
                str += add("BDAY:" + d.getFullYear() + "-" + _.pad((d.getMonth() + 1), 2, "0") + "-" + _.pad(d.getDate(), 2, "0") + CRLF);
            }
            // footer
            str += "END:VCARD";
            return str;
