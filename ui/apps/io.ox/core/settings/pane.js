...
define('io.ox/core/settings/pane', [
    ...
    // savepoints
    (function () {
        if (settings.isConfigurable('features/storeSavePoints')) {
            point.extend({
                id: 'restore',
                index: 900,
                className: 'form-group',
                render: function () {
                    this.$el.append(
                        $('<div class="col-sm-offset-4 col-sm-8">').append(
                            $('<div>').addClass('checkbox').append(
                                $('<label class="control-label">').text(gt('Enable app restore points')).prepend(
                                    new miniViews.CheckboxView({ name: 'features/storeSavePoints', model: this.baton.model, default: true }).render().$el
                                )
                            )
                        )
                    );
    ...
    // mail savepoints
    (function () {
        if (settings.isConfigurable('features/storeMailSavePoints')) {
            point.extend({
                id: 'restoreMail',
                index: 1000,
                className: 'form-group',
                render: function () {
                    var mailCheckbox;
                    this.$el.append(
                        mailCheckbox = $('<div class="col-sm-offset-4 col-sm-8">').append(
                            $('<div>').addClass('checkbox').append(
                                $('<label class="control-label">').text(gt('Enable app restore points for mail compose')).prepend(
                                     new miniViews.CheckboxView({ name: 'features/storeMailSavePoints', model: this.baton.model, defaultVal: true }).render().$el
                                )
                            )
                        ).toggle(!!this.baton.model.get('features/storeSavePoints'))
                    );

                    this.baton.model.on('change:features/storeSavePoints', function (value) {
                        mailCheckbox.toggle(value);
                    });
                }
            });
        }
