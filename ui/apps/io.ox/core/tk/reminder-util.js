...
define('io.ox/core/tk/reminder-util', ['gettext!io.ox/core',
                                       'less!io.ox/core/tk/reminder-util.less'], function (gt) {
            ...
            node.append(
                    $('<div>').addClass('dropdown').css({float: 'left'}).append(
                        ...
                        $('<ul>').addClass('dropdown-menu dropdown-left').css({minWidth: 'auto'}).append(function () {
                            var ret = '';
                            for (var i = 0; i < values.length; i++) {
                                ret += '<li><a ref="#" data-action="reminder" data-value="' + values[i][0] + '">' +
                                    values[i][1] +
                                    '</a></li>';
                            }
                            return ret;
    ...
    var draw = function (node, model, options) {
        ...
        node.attr('data-cid', model.get('cid')).attr('model-cid', model.cid).addClass('reminder-item')
            .append(
                info,
                actions
            );
