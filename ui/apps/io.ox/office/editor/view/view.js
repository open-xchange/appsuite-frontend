...
define('io.ox/office/editor/view/view',
    ...
    function EditorView(app) {
        ...
        toolPane.createToolBar('format', { label: gt('Format') })
            ...
            .addSeparator()
            .addButton('character/bold',      { icon: 'docs-bold',      tooltip: gt('Bold'),           toggle: true })
            .addButton('character/italic',    { icon: 'docs-italic',    tooltip: gt('Italic'),         toggle: true })
            .addButton('character/underline', { icon: 'docs-underline', tooltip: gt('Underline'),      toggle: true })
            .addButton('character/strike',    { icon: 'docs-strikeout', tooltip: gt('Strike through'), toggle: true })
            .addSeparator()
            .addGroup('character/vertalign', new RadioGroup({ toggleValue: 'baseline' })
                .addOptionButton('sub',      { icon: 'docs-subscript',   tooltip: gt('Subscript') })
                .addOptionButton('super',    { icon: 'docs-superscript', tooltip: gt('Superscript') }))
            .addSeparator()
            ...
            .addSeparator()
            .addGroup('paragraph/alignment', new RadioGroup({ icon: 'docs-align-left', tooltip: gt('Paragraph alignment'), dropDown: true, highlight: true, updateCaptionMode: 'icon' })
                .addOptionButton('left',    { icon: 'docs-align-left',    label: gt('Left') })
                .addOptionButton('center',  { icon: 'docs-align-center',  label: gt('Center') })
                .addOptionButton('right',   { icon: 'docs-align-right',   label: gt('Right') })
                .addOptionButton('justify', { icon: 'docs-align-justify', label: gt('Justify') }))
            .addSeparator()
            .addGroup('paragraph/lineheight', new RadioGroup({ icon: 'docs-line-spacing-1', tooltip: gt('Line spacing'), dropDown: true, highlight: true, updateCaptionMode: 'icon' })
                .addOptionButton(LineHeight.SINGLE,   { icon: 'docs-line-spacing-1',   label: gt('Single') })
                .addOptionButton(LineHeight.ONE_HALF, { icon: 'docs-line-spacing-1-5', label: gt('One and a half') })
                .addOptionButton(LineHeight.DOUBLE,   { icon: 'docs-line-spacing-2',   label: gt('Double') }))
            .addSeparator()
            .addGroup('paragraph/fillcolor', new Controls.ColorChooser(editor, 'fill', { icon: 'icon-tint', tooltip: gt('Paragraph Fill Color') }))
            .addSeparator()
            .addButton('character/hyperlink', { icon: 'docs-hyperlink', tooltip : 'Hyperlink' })
            .addSeparator()
            .addButton('paragraph/list/bullets',   { icon: 'docs-bullets',        tooltip: gt('Bullets on/off'),   toggle: true })
            .addButton('paragraph/list/numbering', { icon: 'docs-numbering',      tooltip: gt('Numbering on/off'), toggle: true })
            .addButton('paragraph/list/decindent', { icon: 'docs-num-dec-indent', tooltip: gt('Demote one level') })
            .addButton('paragraph/list/incindent', { icon: 'docs-num-inc-indent', tooltip: gt('Promote one level') })
            .addSeparator();
        ...
        sidePane.createToolBox('insert', { label: gt('Insert') })
            ...
            .addSeparator()
            .addButton('image/insert/file', { icon: 'docs-image-insert', tooltip: gt('Insert image file') })
            .addButton('image/insert/url',  { icon: 'docs-image-insert', tooltip: gt('Insert image URL') });

        sidePane.createToolBox('table', { label: gt('Table') })
            .addButton('table/insert/row',    { icon: 'docs-table-insert-row',    tooltip: gt('Insert row') })
            .addButton('table/insert/column', { icon: 'docs-table-insert-column', tooltip: gt('Insert column') })
            .addButton('table/delete/row',    { icon: 'docs-table-delete-row',    tooltip: gt('Delete selected rows') })
            .addButton('table/delete/column', { icon: 'docs-table-delete-column', tooltip: gt('Delete selected columns') });

        sidePane.createToolBox('drawing', { label: gt('Drawing') })
            .addButton('drawing/delete', { icon: 'docs-image-delete', tooltip: gt('Delete drawing object') })
            .addSeparator()
            .addGroup('drawing/floatmode', new RadioGroup({ icon: 'docs-image-inline', tooltip: gt('Drawing position'), dropDown: true, highlight: true, updateCaptionMode: 'icon' })
                .addOptionButton('inline',       { icon: 'docs-image-inline',      label: gt('Inline with text') })
                .addOptionButton('leftFloated',  { icon: 'docs-image-float-left',  label: gt('Left aligned, text wraps at right side') })
                .addOptionButton('rightFloated', { icon: 'docs-image-float-right', label: gt('Right aligned, text wraps at left side') })
                .addOptionButton('noneFloated',  { icon: 'docs-image-center',      label: gt('Centered, no text wrapping') }));
