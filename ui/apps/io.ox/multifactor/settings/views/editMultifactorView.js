...
define('io.ox/multifactor/settings/views/editMultifactorView', [
    ...
    function openModalDialog(device) {
        ...
        .addButton({ label: gt('Rename'), action: 'ok' })
        .on('ok', function () {
            var dialog = this;
    ...
    ext.point(POINT).extend(
        {
            ...
            render: function (baton) {
                ...
                var label = $('<label for="newName">').append(
        ...
        },
        {
            ...
            id: 'newName',
            render: function () {
                var input = $('<input type="text" id="newName">');
    ...
    function doEdit(model, newName) {
        ...
        var def = $.Deferred();
        api.editDevice(model.get('provider'), model.get('id'), newName).then(def.resolve, function () {
