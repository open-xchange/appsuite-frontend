define('plugins/portal/twitter/util',
    ...
    var renderTweet = function (tweet, opt) {
        ...
        $myTweet.append(
            $('<a>').attr({ href: profileLink, target: '_blank', rel: 'noopener' }).append(
                $('<img>', { src: tweet.user.profile_image_url_https, 'class': 'profilePicture', alt: tweet.user.description})
            ),
            $('<div class="text">').append(
                $('<strong class="io-ox-twitter-name">').append($('<a>', { href: profileLink, rel: 'noopener', target: '_blank'}).text(tweet.user.name)),
                '<br />',
                $('<a>', {'class': 'name', href: profileLink, target: '_blank', rel: 'noopener'}).text('@' + tweet.user.screen_name),
                '<br />',
